{
    ...
    maxPdis: 5000, // massimo numero di punti
    
    // percorso delle icone
    icons: '/admove-agenzie/assets/img/maps/',
    
    // layout base del popup del singolo punto
    infoContent: '<div id="content"><div class="info" id="info{id}"><h2 class="title">-</h2><div class="fl-content"><div class="ico"><img class="media-object" src="assets/img/pdv-sidebar.png" alt=""></div><div class="desc color-blue"><b class="address fs-16">-</b></div></div><div class="data color-blue"></div><div class="buttons mt-35"><a data-id="0" class="change-tab no-bg" title="Evidenzia in tabella"><img src="assets/img/ico-tabella.png"></a><a data-id="0" data-maps="update-raggi" class="hide" title="Evidenzia i raggi del PDV"><span>Evidenzia i raggi del PDV</span></a></div></div></div>',
    ...
    text: { // strings
        ...
        alert_max_pdis: 'Ricordati che non puoi aggiungere più di {tot} punti.',
        txt_yes: 'Sì',
        txt_no: 'No'
    },
    ...
    pdi: { // pdi base
        ...
        _setMarkerIcon: function(opt){ 
            ...
            var label = 'LABEL', ic = {path: google.maps.SymbolPath.CIRCLE,scale: 10,strokeWeight: 0,strokeOpacity:0,rotation:icon.rotation,anchor: new google.maps.Point(0, 3)};
        ...
        setInfoContent: function(html) {
            ...
            // dati aggiuntivi
            var insights = this.insights.data,
                targetName = this.budget.target || '-',
                totalTarget = this.budget.reach,
                budget = CMS.formatPrice(this.budget.value),
                aree = 1,
                data = '';
            ...
            if(this.scope && this.scope.level == 'MobUsers' && this.scope.flow == '2'){ 
               
                //density index
                data += '<div class="mt-5 fl-content"><div class="ico"><img src="/assets/img/ico-mobile-off.png" class="mr-10"></div><div class="desc"><b class="fs-15">Density Index:</b><div class="fs-12 color-grey">Raffronta la distribuzione dei mobile users intorno a questo punto rispetto alla distribuzione intorno agli altri punti della campagna.</div><div class="mt-10 di di'+insights.DensityPotentialUsersNumber+' screw-content">';
                for(var i = 1; i<=16; i++){
                    var cls = (i<=insights.DensityPotentialUsersNumber) ? 'active' : '';
                    data += '<div class="part screw '+cls+'"></div>';
                }
                data += '</div><div class="di-number" style="margin-left:calc(100%/16*'+(insights.DensityPotentialUsersNumber-1)+');">'+insights.DensityPotentialUsersNumber+'</div></div></div>';
            
            }
            
            //split uomo/domma
            data += '<div class="mt-10 fl-content"><div class="ico"><img src="/assets/img/ico-discount.png" class="mr-10"></div><div class="desc"><b class="fs-15">Split Genere</b>';
                data += '<div class="di screw-content mt-10"><div class="part screw gender male" style="float:left;width:'+insights.MaleStatsPercentage+'%"></div><div class="part screw gender female" style="float:left;width:'+insights.FemaleStatsPercentage+'%"></div></div>';
                data += '<div class="di mt-5"><div class="text male uppercase fs-12" style="float:left">Uomini ('+Math.ceil(insights.MaleStatsPercentage)+'%)</div><div class="text female uppercase fs-12" style="float:right">Donne ('+Math.ceil(insights.FemaleStatsPercentage)+'%)</div></div>';
            data += '</div>';
            // Area di pianificazione
            //data += '<div class="mt-10"><img src="/assets/img/ico-gps.png" class="mr-10">N° aree di pianificazione: <b>' + aree + '</b></div>';

            this.info.find('.data').empty().html(data);
    ...
    addPoint: function(opt, stopBounds, duplicated){ // add point
        ...
        pdi.create(pdi.lat,pdi.lng,pdi.radius == undefined ? pdi.circle.radius : pdi.radius);   
    ...
    actions: function(evt,id,data){ // 
        ...
        if (!pdi && evt !== 'add' && evt !== 'added-all') {
            return t.callback(evt, false);
        }
    ...
    centerMap: function(pdi){
        if (pdi) {
            this.map.setCenter(pdi.marker.getPosition());
            this.map.setZoom(pdi.radius == 1500 ? 14 : pdi.radius == 3000 ? 13 : 11);
        }
    },
